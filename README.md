...
| What | Path | Status |
|---|---|---|
| Profile photo | `public/profile.png` | ✅ Your uploaded photo is already here. Replace this file to change it — falls back to a "V" avatar automatically if the file is ever missing. |
...
The production URL is currently set to `https://vasanth-portfolio-fe.vercel.app` in `app/layout.tsx`, `app/sitemap.ts`, and `app/robots.ts`. Update those files only if the deployment domain changes.
...
public/
  profile.png, Vasanth_Kumar_Resume.pdf, favicon.ico, projects/*.png
```
